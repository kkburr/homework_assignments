...
puts "If we delete 5, 6 and 7 from the array, we're left with [#{array_practice.join(',')}]"
puts "If we add 5 at the beginning of the array, we're left with [#{array_practice.unshift(5).join(',')}]"
puts "If we add 6 at the end of the array, we're left with [#{array_practice.push(6).join(',')}]"
puts "Only the elements [#{array_practice.select! {|i| i >8 }.join(', ')}] are > 8."
